...
The system's one true custom component; every shift, on every surface, staff or admin, is a `.ticket`. Perforated top and torn bottom (see Shapes), 1.5px `ink-line` border that turns stamp-red on `.today`/`.on-duty`, dashed internal row dividers, a head row pairing the sans-set day name against the mono-set date. The **hero variant** (`.ticket.hero`, today's card on the staff "Mein Plan" screen) is not a modifier class alone — it is enlarged (1.4rem/1.6rem padding vs. 1.1rem/1.25rem) and its day/date type step up a size, so today is unmistakably the largest object on the screen, not just an outlined ticket in its normal weekday slot. On `admin/shifts.php`, a shift ticket's row also carries a "Bearbeiten" action (title, location, date, times, needed count, note) that opens a `?edit=<id>` form above the rail — the one place a shift's own fields, as opposed to its assignments, are changed after creation.
...
- **Admin decision surface:** approving or rejecting a *pending* application only ever happens inside the ticket rail on `admin/shifts.php`. `admin/applications.php` is a deliberately read-only, week-spanning history view — its own code comment states this is intentional. `admin/calendar.php` (see Components) is a scoped exception, not a second decision surface: it only ever moves an *already-approved* assignment (reschedule its date, or swap which employee holds it) via drag, and its `.chip-pending` badge only ever shows a count and links out — it never has its own approve/reject controls for a pending application, and never can, that decision only happens on `admin/shifts.php`.
